...
internal class MetaXRAcousticMapEditor : Editor
{
            ...
            using (new EditorGUI.DisabledScope(acousticMap.Computing))
            {
                ...
                if (showMappingConfig)
                {
                    ...
                    using (new EditorGUI.DisabledGroupScope(!customPointsEnabled))
                    {
                        ...
                        {
                            EditorGUILayout.PrefixLabel(" ");
                            if (GUILayout.Button(new GUIContent("Prune map", "Automatically remove out of bounds points.")))
                                pruneScene = true;
                        }
                    }
    ...
    private const bool AGGRESSIVE_OUT_OF_MAP_PRUNING = false; //prune if any ray doesn't hit geometry (useful for dense indoor spaces)
    
    private void PruneScene()
    {
        acousticMap = target as MetaXRAcousticMap;
        int pointCount = acousticMap.PointCount;
        
        Debug.Log($"pruning {pointCount} points");
        Physics.queriesHitBackfaces = true;
        
        for (int i = 0; i < pointCount; i++) {
            Vector3 point = acousticMap.GetPoint(i);
            Collider[] cols = Physics.OverlapSphere(point, 0.01f, ~0, QueryTriggerInteraction.Ignore);
            if (cols.Length > 0) {
                acousticMap.RemovePoint(i);
                pointCount--;
                i--;
                continue;
            }
            ...
            foreach (var dir in directions) {
                if (Physics.Raycast(point, dir, out RaycastHit hit, Mathf.Infinity, ~0, QueryTriggerInteraction.Ignore)) {
                    hits++;
                ...
                else if (AGGRESSIVE_OUT_OF_MAP_PRUNING) {
                    acousticMap.RemovePoint(i);
                    pointCount--;
                    i--;
                    hits = 1;
